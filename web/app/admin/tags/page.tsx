    ...
    <>
      ...
      <main className="p-8 max-w-3xl">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-slate-800">Tag Definitions</h1>
          <button onClick={handleRunTagging} disabled={running}
            className="bg-teal-700 text-white text-sm px-4 py-2 rounded-lg hover:bg-teal-800 disabled:opacity-50">
        ...
        <form onSubmit={handleAdd} className="flex gap-2 mb-6">
          <input placeholder="Tag name" value={name} onChange={e => setName(e.target.value)}
            className="border border-slate-200 rounded px-3 py-2 text-sm flex-shrink-0 w-40" />
          <input placeholder="Description" value={description} onChange={e => setDescription(e.target.value)}
            className="border border-slate-200 rounded px-3 py-2 text-sm flex-1" />
          <button type="submit" disabled={adding}
            className="bg-teal-700 text-white text-sm px-4 py-2 rounded-lg hover:bg-teal-800 disabled:opacity-50">Add</button>
        </form>
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {tags.map(tag => (
              <tr key={tag.id} className={`border-b border-slate-100 ${tag.retired ? 'opacity-40' : ''}`}>
                <td className="py-3 text-slate-800 font-mono text-xs">
                  {tag.name}
                  {tag.retired && <span className="ml-2 text-xs font-sans text-slate-400 font-normal">retired</span>}
                </td>
                  ...
                  {editingId === tag.id ? (
                    ...
                    <span className="flex gap-2 items-center group">
                      ...
                      <button onClick={() => { setEditingId(tag.id); setEditDesc(tag.description ?? ''); }}
                        className="opacity-0 group-hover:opacity-100 text-xs text-slate-400 hover:text-slate-700">Edit</button>
                    </span>
                  )}
                </td>
                <td className="py-3 text-right">
                  {tag.retired ? (
                    <button onClick={() => handleRetire(tag.id, false)}
                      className="text-slate-500 hover:text-slate-700 text-xs border border-slate-200 rounded px-2 py-1">Unretire</button>
                  ) : (
            ...
            {tags.length === 0 && (
              <tr><td colSpan={3} className="py-8 text-center text-slate-400">No tags yet</td></tr>
            )}
          </tbody>
        </table>
      </main>
